  ...
  return (
    <div className="ru-header">
      <div className="ru-logo-row">
        <img src={logo} alt="Reinvent Uniforms" className="ru-logo-img" />
      </div>
    ...
    <div className="ru-stepbar">
      {labels.map((label, i) => {
        ...
        return (
          <div
            key={n}
            className={`ru-step ${active ? "active" : ""} ${done ? "done" : ""}`}
          >
...
function Screen1({ onNext }) {
  ...
  return (
    <div className="ru-card">
      ...
      {error && (
        <p className="ru-error">Please fill in all fields to continue.</p>
      )}

      <button className="ru-btn-primary" onClick={handleNext}>
        Continue →
      </button>
    </div>
  );
...
function Screen2({ selection, onNext, onBack }) {
  ...
  return (
    <div className="ru-card">
      <button className="ru-btn-back" onClick={onBack}>
        ← Back
      </button>

      <h2 className="ru-card-heading">Take measurements</h2>
      <p className="ru-card-sub">
        {selection.gender} · {selection.grade} ·{" "}
        {selection.branch.split("–")[1]?.trim() ?? selection.branch}
      </p>

      <div className="ru-measure-grid">
        {/* Height */}
        <div className="ru-measure-card">
          <img
            src={IMG_HEIGHT}
      ...
      {error && (
        <p className="ru-error">
          Please enter both height and chest measurements.
        </p>
      )}
...
function Screen3({ measurements, onBack, onRestart }) {
  const { height, chest,waist } = measurements;
  ...
  return (
    <div className="ru-card ru-card-result">
      ...
      <div className="ru-result-banner">
        ...
        <p className="ru-result-meas">
          Height {height} cm · Chest {chest} cm
        </p>
      </div>
        ...
        <table className="ru-table">
          ...
          <tbody>
            {SIZE_DATA.map((row) => (
              <tr
                key={row.size}
                className={row.size === size ? "ru-row-match" : ""}
              >
                <td>
      ...
      <button className="ru-btn-secondary" onClick={onRestart}>
        ↺ Measure again
      </button>
    </div>
  );
...
export default function App() {
  ...
  function handleScreen1(data) {
    setSelection(data);
    setStep(2);
  }
  function handleScreen2(data) {
